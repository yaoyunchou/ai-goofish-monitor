...
### feat(xhs): 笔记单独采集，账号按渠道分开，渠道改为线程池

- 侧栏「小红书」增加「笔记」。笔记必须使用小红书登录，上海时区每天一行，同一天再采覆盖当天；没读到的数字保留。商品公开页仍然不登录、快照只追加
- 商品添加遇到笔记链接时不写入商品表，并提示到笔记菜单添加。失败列表里已有的笔记不自动搬走
- 账号增加渠道：闲鱼、小红书。已有 `state/*.json` 登记为闲鱼。小红书登录文件放在 `state/xhs/`。闲鱼任务只列出闲鱼账号
- 渠道执行改为最多 5 个同时运行。同一渠道仍然串行。没有任务时不留空闲线程。线程退出后回收占用，日志在 `logs/channel_workers.log`
- 回归：`tests/unit/test_channel_workers.py`、`tests/unit/test_xhs_notes.py`、`tests/integration/test_api_accounts.py`

## 2026-09-23

### fix(xhs): 采集时换页面不再卡住

- 现象：小红书采集进行时，点侧栏去别的页面会一直转，要等这轮采集结束
- 原因：公开页浏览器走 Playwright 同步接口，放在 `asyncio.to_thread` 里仍会拖住主事件循环，别的接口请求排不出去
- 处理：小红书采集只在自己的渠道线程里跑，不再交回事件循环
- 回归：`tests/unit/test_channel_workers.py`

### fix(xhs): 失败列表的重试一点就有反馈

- 现象：点「重试」按钮不变，再点提示「该渠道正在采集」，结束后这条还在失败列表
- 原因：重试要等这一轮采集结束才提示，期间按钮仍可点。这条失败是笔记链接 `/discovery/item/`，不是商品页，重试也不会出现累计已售
- 处理：一点击就变成「采集中…」并禁止再点。笔记链接立刻写明原因，不再打开页面，也不把后面的商品停掉
- 回归：`tests/unit/test_xhs_analytics.py`

### fix(xhs): 公开页等渲染完再读已售

- 现象：9 个商品全部失败。先是 `SSL: UNEXPECTED_EOF_WHILE_READING`（Python 走了本机代理 `127.0.0.1:7897`，握手被掐断）。稍后连上之后又变成「公开页未解析到累计已售」
- 原因：商品页现在是前端渲染。直接下载只有 `<div id="app">` 空壳，已售、价格、标题都在脚本画出来之后才出现。快照表一直是 0 行
- 处理：小红书这条执行线用单独的浏览器打开公开页，不走本机代理，不带登录态，等 `.spu-text` 出现后再解析。商品已售取 `spu-text`，不用店铺那条已售。笔记链接 `/discovery/item/` 改开商品页，避免它的登录墙把后面的商品停掉
- 回归：`tests/unit/test_xhs_analytics.py`

### fix(xhs): 采集中在看板上看得见

- 点「采集全部」或单条「采集」后，按钮立刻变成「采集中…」，并出现提示条
- 定时或另一次采集已经在跑时，看板每 3 秒看一次状态，不用等这次点击返回也能看见
- 回归：`tests/unit/test_channel_workers.py`

### feat(scheduler): 按渠道分开执行

- 闲鱼（关键词、卖家订阅、店铺罗盘）一条执行线，同一时间只开一个浏览器，后面的定时任务排队
- 小红书公开页采集走另一条线，不等闲鱼，也不会堵住闲鱼的调度
- 手动再点同一个渠道时立刻返回「该渠道正在采集」
- 接入步骤见 `docs/design/channel-execution.md`
- 回归：`tests/unit/test_channel_workers.py`

## 2026-09-22

### feat(xhs): 失败、下架和设置各自一页

- 商品看板只留在监控中的商品和采集。定时采集挪到「设置」
- 「失败列表」放这次没采到的商品，可以重试、忽略或删除。忽略后回到看板
- 「下架列表」只回看页面写明已下架或违规的商品，不再采集，历史快照还在
- 回归：`tests/unit/test_xhs_analytics.py`、`tests/unit/test_xhs_shops.py`

### feat(xhs): 添加商品单独成页

- 侧栏「小红书」增加「添加商品」。每行一个链接或商品 ID，店铺、分类和标记套用到这一批
- Excel 模板下载和导入从商品看板挪到这一页。看板上只留采集、定时和商品表
- 回归：`web-ui/src/router/routes.smoke.test.ts`、`web-ui/src/i18n/smoke.test.ts`

### feat(xhs): 店铺菜单、分类标记和 Excel 导入

- 侧栏「小红书」增加「店铺」。按店看监控商品的累计已售合计，以及今日、昨日、上小时
- 添加商品可填店铺、分类和标记。看板上能加、删标记。手选店铺不会被页面店名改掉
- 下载 `.xlsx` 模板后按行导入。空格子不覆盖已有店铺、分类和标记
- 回归：`tests/unit/test_xhs_shops.py`

### fix(xhs): 定时采集补上每天、每周、每月

- 频率增加「每天 / 每周 / 每月」。每周选星期几，每月选几号，再选钟点
- 每周存成 `mon`–`sun`。当前调度器把数字 0 当成周一，不用标准 Cron 的 0=周日
- 每月 29–31 号会提示：没有这一天的月份会跳过
- 回归：`web-ui/src/lib/xhsSchedule.test.ts`

### fix(xhs): 定时采集改成选频率或每天几点

- 商品看板上的 Cron 输入框改成「每 15 分钟 / 每小时 / 每 2 小时…」或「每天固定时间」
- 每天固定时间用钟点选择。下次采集显示今天、明天或日期，不再直接露出表达式
- 不在常用选项里的已存规则会保留，另选频率并保存后才替换
- 回归：`web-ui/src/lib/xhsSchedule.test.ts`

### fix(start): 源码更新后不再跳过前端构建

- 现象：后端已起来，侧栏没有「小红书」。源码里有菜单，`dist` 仍是旧包
- 原因：`start.bat` 用 `%%~tf` 和自己比较，条件永远不成立，只要 `dist\index.html` 存在就跳过 `npm run build`
- 处理：按 `web-ui\src` 里比 `dist\index.html` 更新的文件决定是否重新构建

### feat(xhs): 侧栏接入小红书公开商品监控

- 独立菜单 `/xhs`：添加公开链接、看板（累计已售 / 今日 / 昨日 / 上小时）、单品按小时与近 7 日变化
- 变化用高水位差值。缺基线显示「—」，中途开始监控标口径不完整。快照只追加
- 采集不带小红书登录态。公开页 461 或要求登录时停止本轮
- 定时 job `xhs_monitor`，默认关闭，`misfire_grace_time=3600`，`reload_jobs` 不会卸掉
- 闲鱼想要/浏览的日覆盖和周差值本期不改。对照见 `docs/design/metric-delta-comparison.md`
