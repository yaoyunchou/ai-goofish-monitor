...
| [账号按渠道分开](./account-channels.md) | **本期实现** | `monitor_accounts`；闲鱼 / 小红书分渠道；Cookie 仍在文件 |
| [渠道线程池](./channel-pool.md) | **本期实现** | 同时最多 5 个渠道任务；同渠道串行；线程退出回收 |
| [小红书笔记](./xhs-notes.md) | **本期实现** | 独立菜单；登录采集；上海日一行覆盖。不改商品公开页 |
