...
| 渠道执行 | 同时最多 5 个渠道任务；同一渠道仍然串行。没有任务时不留空闲线程。线程退出后回收占用。日志在 `logs/channel_workers.log` |
...
独立侧栏分组。只采集公开商品页，不使用小红书登录态。商品页先由浏览器画出来，再读取页面上的已售；空壳 HTML 里没有这个数字。
...
| GET | `/api/xhs/collect-status` | 小红书这条执行线是否正在采集 |
