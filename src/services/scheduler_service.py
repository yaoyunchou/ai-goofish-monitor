...
XHS_NOTE_JOB_ID = "xhs_note_monitor"
...
class SchedulerService:
    ...
    def get_xhs_note_next_run_time(self):
        return self._job_next_run_time(XHS_NOTE_JOB_ID)

    async def reload_xhs_note_job(self, schedule: dict):
        """加载笔记采集。reload_jobs 只删 task_*，不会卸掉本 job。"""
        existing = self.scheduler.get_job(XHS_NOTE_JOB_ID)
        if existing is not None:
            self.scheduler.remove_job(XHS_NOTE_JOB_ID)
        if schedule.get("enabled") and schedule.get("cron"):
            try:
                trigger = build_cron_trigger(
                    schedule["cron"],
                ...
                self.scheduler.add_job(
                    ...
                    trigger=trigger,
                    id=XHS_NOTE_JOB_ID,
                    name="Scheduled: xhs notes",
                    **self._cron_job_options(),
                )
                print(f"  -> 已为小红书笔记添加定时规则: '{schedule['cron']}'")
            except ValueError as exc:
                print(f"  -> [警告] 小红书笔记 Cron 无效: {exc}")

    async def _run_xhs_notes(self):
        import asyncio

        from src.services.channel_workers import get_channel_workers
        from src.services.xhs_note_storage import collect_notes

        print("定时任务触发: 正在采集小红书笔记...")
        try:
            _status, done = get_channel_workers().submit(
                "xhs_note", collect_notes, wait=True, on_thread=True
            )
